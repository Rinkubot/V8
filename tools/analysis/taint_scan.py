...
def is_unbounded_source_expr(expr: str) -> bool:
	...
	for s in UNBOUNDED_SOURCES:
		if s.replace(' ', '') in expr_ws:
			# Treat fgets as unbounded ONLY if its len argument is a non-constant
			# or omitted (in which case it is unsafe). Here we cannot robustly parse,
			# so we conservatively accept getenv/gets/getline/std::getline as unbounded
			# and treat fgets/read/recv as unbounded only when used directly without
			# any length bounds in sinks like strcpy/sprintf.
			return True
	return False
	...
	for lineno, line in enumerate(lines, start=1):
		...
		if mfunc:
			...
			for p in split_args(param_blob):
				...
				if pm:
					# parse reversed to better catch the last identifier
					name_rev = pm.group(1)
					name = name_rev[::-1]
		...
		# Consider only straightforward calls
		...
		if any(callee_ws.startswith(s[:-1]) for s in UNSAFE_SINKS):
			if callee_ws.startswith('gets'):
				# gets(dest) is always unsafe if dest is a fixed buffer
			...
			# For strcpy/strcat/sprintf: require that the source argument is unquestionably tainted
			# and dest is a fixed-size local buffer.
			if callee_ws.startswith('strcpy') or callee_ws.startswith('strcat'):
				if len(args) >= 2 and args[0] in buffer_sizes:
					src = args[1]
					# Tainted if direct param name or direct unbounded source call
			...
			if callee_ws.startswith('sprintf') or callee_ws.startswith('vsprintf'):
				# sprintf(dst, fmt, ...) — if dst is fixed and a tainted argument is used for fmt or as a %s, it's risky.
				if args and args[0] in buffer_sizes:
...
def scan_tree(root: str, include_tests: bool = False) -> List[Dict]:
	...
	for dirpath, dirnames, filenames in os.walk(root):
		# Skip common large third_party/ directories and build outputs
		skip = any(part in (
